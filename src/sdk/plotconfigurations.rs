...
pub struct IAcadPlotConfigurations {
    disp:ComObject
}

impl IAcadPlotConfigurations{
    ...
    pub fn Item(&self,index:u8)->Option<IAcadPlotConfiguration>{
        ...
        match hres.to_idispatch(){
            Ok(disp)=>{
                let acad_doc = IAcadPlotConfiguration::new(disp.clone());
